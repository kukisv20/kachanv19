...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `olá ◤◢ @${num.split('@')[0]} ◤◢◣\nbem vindo ao grupo:n/━━━━❰･ *${mdata.subject}* ･❱━━━━\n\nPor favor leia as regras arrombado(a)\n E seja feliz no grupo🤗️`
				...
				teks = `Flw @${num.split('@')[0]} arrombado(a) 😣✋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const premium = ["5522988714803@s.whatsapp.net","447541362463@s.whatsapp.net","5511968637046@s.whatsapp.net","558191015788@s.whatsapp.net","558188836654@s.whatsapp.net","5511963018928@s.whatsapp.net","5522992161220@s.whatsapp.net","5511948981159@s.whatsapp.net","554899974618@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("ma1287caco")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('5522988714803@s.whatsapp.net', options, text, {quoted: mek})
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO LUCI BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @luci domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "luci"🙄, chama ele aí ele diz que é "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT LUCI ✨* 」\n*│+ wa.me/5522988714803*╰──────*「 *NABUTO* 」*────*\n\n*_SE QUER SER ADMIN DO BOT NABUTO_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/556993733829*╰──────*「 * LUCI* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO LUCI BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* LUCI\n*WPP:* wa.me/+5522988714803\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
